<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="85px">
      <el-form-item label="渠道公司" prop="companyName">
    ...
    <el-dialog :title="title" :visible.sync="open" width="80%" :close-on-click-modal="false" append-to-body>
              ...
              <el-form-item label="业务范围" prop="businessScope">
                <el-select v-model="form.businessScope" placeholder="请选择公司业务范围" clearable>
                  <el-option v-for="dict in dict.type.crm_companny_business_scope" :key="dict.value" :label="dict.label"
                    :value="dict.value" :disabled="!dict.value.includes('SI')" />
                </el-select>
              </el-form-item>
            </el-col>
          </el-row>
      ...
      <el-steps :active="companyDetailActive" align-center simple v-if="baseCompanyReadOnly">
        <el-step title="渠道关联商机查看" icon="el-icon-upload" @click.native="changeStep(1)" />
        <el-step title="渠道联系人管理" icon="el-icon-picture" @click.native="changeStep(2)" />
        <el-step title="渠道关联项目/应用" icon="el-icon-picture" @click.native="changeStep(3)" />
      </el-steps>
      <div id="stepsArea" v-if="baseCompanyReadOnly">
        ...
        <div id="channelContactArea" v-if="companyDetailActive === 2">
          <el-table v-loading="loading" :data="channelContactList" key="two">
            ...
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
              <template slot-scope="scope">
                <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdateChannelContact(scope.row)"
                  v-hasPermi="['crm:company:edit']">修改
                </el-button>
                <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDeleteChannelContact(scope.row)"
                  v-hasPermi="['crm:company:remove']">删除
                </el-button>
              </template>
            </el-table-column>
          </el-table>
          <pagination v-show="channelContactTotal > 0" :pageSizes="[4, 6, 8]" :total="channelContactTotal"
            :page.sync="queryChannelContactParams.pageNum" :limit.sync="queryChannelContactParams.pageSize"
            @pagination="queryChannelContactList" />
          <div style="text-align: center;">
            <el-button type="primary" round @click="handleAddChannelContact()">添加新联系人</el-button>
          </div>
        </div>
        <div id="channelAppArea" v-if="companyDetailActive === 3">
          <el-table v-loading="loading" :data="channelAppList" key="third">
            ...
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
              <template slot-scope="scope">
                <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdateChannelApp(scope.row)"
                  v-hasPermi="['crm:company:edit']">修改
                </el-button>
                <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDeleteChannelApp(scope.row)"
                  v-hasPermi="['crm:company:remove']">删除
                </el-button>
              </template>
            </el-table-column>
          </el-table>
          <pagination v-show="channelAppTotal > 0" :pageSizes="[4, 6, 8]" :total="channelAppTotal"
            :page.sync="queryChannelAppParams.pageNum" :limit.sync="queryChannelAppParams.pageSize"
            @pagination="queryChannelAppList" />
          <div style="text-align: center;">
            <el-button type="primary" round @click="handleAddChannelApp()">添加单独项目/应用</el-button>
          </div>
        </div>
      </div>
    ...
    <el-dialog :title="appAreaTitle" :visible.sync="openAppArea" width="40%" :close-on-click-modal="false" append-to-body>
...
export default {
  ...
  data() {
    return {
      ...
      queryChannelAppParams: {
        pageNum: 1,
        pageSize: 4,
      },
      queryChannelContactParams: {
        pageNum: 1,
        pageSize: 4,
      },
      // 表单参数
  ...
  methods: {
    ...
    transferSubmitForm() {
      this.$refs["transfer.form"].validate(valid => {
        if (valid) {
          ...
          var _this = this;
          this.transfer.form.sourceType = "CHAN";
          this.$modal.confirm('是否确认交接编码为"' + codes + '"的渠道项？').then(function () {
        ...
        if (valid) {
          ...
          if (this.form.id != null) {
            ...
            if (!this.form.code) {
              ...
              this.form.code = 'CHAN' + timeStr.replace(" ", rondomStr);
            }
    ...
    handleUpdateChannelContact(row) {
      ...
      getContact(id).then(response => {
        ...
        this.openContactArea = true;
        this.contactAreaTitle = "修改渠道联系人";
      });
    },
    handleUpdateChannelApp(row) {
      this.resetApp();
      const id = row.id;
    ...
    handleDeleteChannelApp(row) {
      const ids = row.id;
      this.$modal.confirm('是否确认删除渠道应用编号为"' + ids + '"的数据项？').then(function () {
    ...
    handleDeleteChannelContact(row) {
      const ids = row.id;
      this.$modal.confirm('是否确认删除渠道联系人编号为"' + ids + '"的数据项？').then(function () {
