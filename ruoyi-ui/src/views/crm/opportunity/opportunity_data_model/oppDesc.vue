<template>
  <div class="app-container" id="oppDesc" :style="{pointerEvents:oppdata.action=='V'?'none':'unset'}">
    <h3>{{ stageTitle }}</h3>
    <el-divider/>
    <el-form ref="oppDescForm" :model="oppDescForm" :rules="rules" size="medium" label-width="220px" v-loading="flag.oppDescLoading">
        ...
        <el-row :gutter="15">
          <el-col :span="8">
            <el-form-item label="客户要求系统正式上线时间" prop="systemPublicTime.propertyVal">
              <el-date-picker
                v-model="oppDescForm.systemPublicTime.propertyVal"
                type="date"
          ...
          <el-col :span="8">
            <el-form-item label="客户召集ISV、SI启动项目时间" prop="startProjectTime.propertyVal">
              <el-date-picker
                v-model="oppDescForm.startProjectTime.propertyVal"
                type="date"
                value-format="yyyy-MM-dd"
                placeholder="选择日期时间">
              </el-date-picker>
        ...
        <el-row :gutter="15">
          <el-col :span="8">
            <el-form-item label="系统项目总预算" prop="projectBudget.propertyVal">
              <el-input v-model="oppDescForm.projectBudget.propertyVal" type="number" placeholder="请输入系统项目总预算金额" clearable>
              </el-input>
          ...
          <el-col :span="8">
            <el-form-item label="项目中DB预算" prop="projectDbBdget.propertyVal">
              <el-input v-model="oppDescForm.projectDbBdget.propertyVal" type="number" placeholder="请输入项目中DB预算金额" clearable>
              </el-input>
        ...
        <el-row :gutter="15">
          ...
          <el-col :span="8">
            <el-form-item label="预计投标日期" prop="preTenderDate.propertyVal">
              <el-date-picker
                v-model="oppDescForm.preTenderDate.propertyVal"
  ...
  data() {
    return {
      ...
      rules:{
        ...
        "matchProduct":{
          propertyVal: [
            { required: true, message: "未选择 对应我司产品", trigger: "blur" }
          ],
        },

        "chosedFlag":{
          propertyVal: [
            { required: true, message: "未选择 入围要求", trigger: "blur" }
          ],
        },"approveFlag":{
          propertyVal: [
            { required: true, message: "未选择 批准立项", trigger: "blur" }
          ],
        },"isFundOk":{
          propertyVal: [
            { required: true, message: "未选择 资金是否到位", trigger: "blur" }
          ],
        },"budgetSource":{
          propertyVal: [
            { required: true, message: "未选择 预算来源", trigger: "blur" }
          ],
        },
        "preTenderDate":{
          propertyVal: [
          ...
          propertyVal: [
            { required: true, message: "未选择 预计招标类型", trigger: "blur" }
          ],
        },
        "preContractVal":{
          propertyVal: [
        ...
        "custInterest":{
          propertyVal: [
            { required: true, message: "未填选择 用户关注点", trigger: "blur" }
          ],
        },        
      },
    }
