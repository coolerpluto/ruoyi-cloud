        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="商机编号" prop="opportunityNum">
              <el-input v-model="formData.opportunityNum" placeholder="请输入商机编号" clearable
          ...
          <el-col :span="8">
            <el-form-item label="最终用户" prop="customerName">
              <el-input v-model="formData.customerName" placeholder="请输入最终用户" clearable
                        :style="{width: '100%'}"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="8">
            <el-form-item label="授权用途" prop="purpose">
              <el-select v-model="formData.purpose" placeholder="请输入授权用途" clearable :style="{width: '100%'}"
                         @change="purposeChanged">
                <el-option v-for="(item, index) in dict.type.medium_lic_purposes" :key="index" :label="item.label"
                           :value="item.value" :disabled="item.disabled"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8" v-show="formData.purpose != 'o'">
            <el-form-item label="授权到期日" prop="expireTime">
              <el-date-picker v-model="formData.expireTime" format="yyyy-MM-dd HH:mm:ss"
          ...
          <el-col :span="8">
            <el-form-item label="产品类型" prop="prodType">
              <el-select v-model="formData.prodType" placeholder="请选择产品类型" clearable :style="{width: '100%'}"
                         @change="prodTypeChanged">
                <el-option v-for="(item, index) in dict.type.medium_lic_prod_type" :key="index" :label="item.label"
                           :value="item.value" :disabled="item.disabled"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8" v-show="formData.prodType == '0'">
            <el-form-item label="数据库版本" prop="dbVersion">
              <el-select v-model="formData.dbVersion" placeholder="请选择数据库版本" filterable :style="{width: '100%'}">
                <el-option v-for="(item, index) in dict.type.medium_db_version" :key="index" :label="item.label"
                           :value="item.value" :disabled="item.disabled"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
...
<script>
import {generatorHGDB458} from "@/api/medium/license";
  ...
  methods: {
    submitForm() {
      this.$refs['elForm'].validate(valid => {
        ...
        generatorHGDB458(this.formData).then(response => {
    ...
    prodTypeChanged(val) {
      if (val === '0') {
        this.formData.dbVersion = 'HGDB-SEE-V4.5.8';
      } else {
