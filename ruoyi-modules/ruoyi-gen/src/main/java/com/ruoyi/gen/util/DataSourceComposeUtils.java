...
public class DataSourceComposeUtils
{
    ...
    private static String URL_TEMPLATE_MARIADB = "jdbc:mariadb://{}:{}/{}?useUnicode=true&characterEncoding=utf8&zeroDateTimeBehavior=convertToNull&useSSL=true&serverTimezone=GMT%2B8";
    /**
     * Oracle的SID连接字符串模板
     */
    ...
    private static String URL_TEMPLATE_HIGHGOSQL = "jdbc:highgo://{}:{}/{}";

    public static DruidDataSource composeDruidDataSource(GenDbSource dataSource) throws ServiceException
    {
        ...
        else if (StringUtils.equals(dataSource.getDbType(), Constants.DATABASE_TYPE_MARIADB))
        {
            druidDataSource.setUrl(StringUtils.format(URL_TEMPLATE_MARIADB, dataSource.getHost(), dataSource.getPort(),
                    dataSource.getName()));
        }
        ...
        else if (StringUtils.equals(dataSource.getDbType(), Constants.DATABASE_TYPE_HIGHGO))
        {
            druidDataSource.setUrl(StringUtils.format(URL_TEMPLATE_HIGHGOSQL, dataSource.getHost(),
                    dataSource.getPort(), dataSource.getName(), dataSource.getSchemaName()));
        }
        else
        {
            throw new ServiceException("数据库驱动类型无效");
        }
